...
$sql = mysqli_query($conn,"select * from rapport");
        ...
        <div class="id">
         ...
         if(mysqli_num_rows($find)>0){
            foreach($find as $found){
                $type_agri = $found['typ_agriculteur'];
...
<?php
    ...
    if($type_agri === 'producteur'){
        $sqlcert = "select * from recolt_produit where id_code = '$id_code'";
        $res = mysqli_query($conn,$sqlcert);
        if(mysqli_num_rows($res)>0){
            foreach($res as $row){
?>
                <p><?=$row['produit']?></p>
    ...
    }elseif($type_agri === 'multiplicateur'){
        $sqlcert = "select * from recolt_semance where id_code = '$id_code'";
        $res = mysqli_query($conn,$sqlcert);
        if(mysqli_num_rows($res)>0){
            foreach($res as $row){
?>
